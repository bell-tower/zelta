...
alias zdbgenv=". $ZELTA_DBG/test_env.sh && . $ZELTA_DBG/set_reuse_tmp_env.sh"
...
```
test/runners/
├── debug_runner.sh      - Main test runner for debugging; runs shellspec tests through 040 with xtrace
...
├── reset_env.sh         - Resets test environment by unsetting SANDBOX_ZELTA_TMP_DIR, checked in test/test_helper.sh
